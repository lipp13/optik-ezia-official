"use client";

import React from "react";
import Image from "next/image";
import Link from "next/link";
import { ArrowRight, CheckCircle2, ShieldCheck, MapPin, Instagram } from "lucide-react";
import { SERVICES_PROCESS, LENS_OPTIONS } from "@/data/services";
import { STORES_DATA } from "@/data/stores";
import { OPTIK_EZIA_CONFIG } from "@/data/optikEzia";

export default function ServicesPage() {
  return (
    <div className="pt-28 pb-32 bg-ivory">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        {/* Page Header */}
        <div className="max-w-3xl pb-12 border-b border-sand-border">
          <span className="editorial-tag text-accent-terracotta block mb-3">
            LAYANAN & PEMERIKSAAN
          </span>
          <h1 className="editorial-headline text-4xl sm:text-6xl text-charcoal">
            Pemeriksaan Mata, Lensa & Fitting Kacamata
          </h1>
          <p className="text-base sm:text-lg font-light text-charcoal-muted mt-4 leading-relaxed">
            Layanan optik terpadu di Optik Ezia untuk memastikan ketajaman penglihatan, kenyamanan lensa, dan kesesuaian posisi bingkai pada wajah Anda.
          </p>
        </div>

        {/* 5-Step Process Breakdown */}
        <div className="py-16 space-y-16">
          <div className="max-w-2xl">
            <span className="editorial-tag text-accent-terracotta block mb-2">
              ALUR 5 TAHAPAN
            </span>
            <h2 className="editorial-headline text-2xl sm:text-4xl text-charcoal">
              Prosedur Layanan Kacamata
            </h2>
          </div>

          <div className="space-y-12">
            {SERVICES_PROCESS.map((step) => (
              <div
                key={step.stepNumber}
                className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-ivory-warm border border-sand-border p-6 sm:p-10"
              >
                <div className="lg:col-span-6 space-y-4">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-mono font-bold tracking-widest text-accent-terracotta bg-sand/60 px-2.5 py-1">
                      TAHAP {step.stepNumber}
                    </span>
                    <span className="text-[11px] uppercase tracking-wider text-charcoal-light">
                      {step.subtitle}
                    </span>
                  </div>

                  <h3 className="text-2xl font-light text-charcoal font-display">
                    {step.title}
                  </h3>

                  <p className="text-sm font-light text-charcoal-muted leading-relaxed">
                    {step.description}
                  </p>

                  <ul className="space-y-2 pt-2 text-xs text-charcoal border-t border-sand-border/80">
                    {step.detailPoints.map((point, idx) => (
                      <li key={idx} className="flex items-center gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5 text-accent-terracotta shrink-0" />
                        <span>{point}</span>
                      </li>
                    ))}
                  </ul>
                </div>

                <div className="lg:col-span-6 relative aspect-[16/10] w-full overflow-hidden bg-sand/40 border border-sand-border">
                  <Image
                    src={step.image}
                    alt={step.title}
                    fill
                    className="object-cover"
                    sizes="(max-width: 1024px) 100vw, 50vw"
                  />
                  <div className="absolute bottom-3 left-3 bg-charcoal/90 text-ivory text-[9px] uppercase tracking-widest px-2.5 py-1">
                    Standar Optik Ezia
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>

        {/* Lens Options Section */}
        <div className="py-16 border-t border-sand-border">
          <div className="max-w-2xl mb-12">
            <span className="editorial-tag text-accent-terracotta block mb-2">
              PILIHAN LENSA
            </span>
            <h2 className="editorial-headline text-2xl sm:text-4xl text-charcoal">
              Kategori Lensa Kacamata
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {LENS_OPTIONS.map((lens) => (
              <div
                key={lens.id}
                id={lens.id}
                className="bg-ivory-warm border border-sand-border p-8 flex flex-col justify-between space-y-6"
              >
                <div>
                  <span className="editorial-tag text-accent-terracotta block mb-2">
                    {lens.tag}
                  </span>
                  <h3 className="text-xl font-light text-charcoal font-display mb-3">
                    {lens.name}
                  </h3>
                  <p className="text-xs text-charcoal-muted leading-relaxed mb-6">
                    {lens.description}
                  </p>

                  <ul className="space-y-2 text-xs text-charcoal border-t border-sand-border pt-4">
                    {lens.benefits.map((b, i) => (
                      <li key={i} className="flex items-center gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5 text-accent-terracotta shrink-0" />
                        <span>{b}</span>
                      </li>
                    ))}
                  </ul>
                </div>

                <Link
                  href="/stores"
                  className="btn-editorial-primary text-center text-[11px] py-3"
                >
                  Konsultasi di Cabang
                </Link>
              </div>
            ))}
          </div>
        </div>

        {/* Consultation Guidance Banner */}
        <div className="mt-16 bg-ivory-warm border border-sand-border p-8 sm:p-12 text-center space-y-6">
          <span className="editorial-tag text-accent-terracotta">
            KUNJUNGAN CABANG
          </span>
          <h2 className="editorial-headline text-3xl sm:text-4xl text-charcoal max-w-xl mx-auto">
            Ingin Melakukan Pemeriksaan Mata?
          </h2>
          <p className="text-sm font-light text-charcoal-muted max-w-lg mx-auto leading-relaxed">
            Kunjungi cabang Optik Ezia terdekat untuk pemeriksaan ketajaman mata dan pemilihan model frame yang tepat.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
            <Link
              href="/stores"
              className="btn-editorial-primary inline-flex items-center gap-2"
            >
              <MapPin className="w-4 h-4 text-accent-gold" />
              <span>Lihat Lokasi Cabang</span>
            </Link>

            <a
              href={OPTIK_EZIA_CONFIG.instagram.url}
              target="_blank"
              rel="noopener noreferrer"
              className="btn-editorial-outline inline-flex items-center gap-2"
            >
              <Instagram className="w-4 h-4 text-accent-terracotta" />
              <span>Instagram {OPTIK_EZIA_CONFIG.instagram.handle}</span>
            </a>
          </div>
        </div>

      </div>
    </div>
  );
}
